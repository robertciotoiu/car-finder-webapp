...
docker build . -t car-finder-webapp:latest
```

Once the Docker image is correctly built, you can test it locally using

```
docker run -p 8080:8080 car-finder-webapp:latest
```
...
If the pods say `Container image "car-finder-webapp:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
